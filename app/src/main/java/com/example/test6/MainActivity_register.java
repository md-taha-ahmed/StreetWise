...
//                                        break;
    ...
    public void insertUserData()
    {
        ...
        reference = FirebaseDatabase.getInstance().getReference().child("USERS").child("CUSTOMER").child(ed2.getText().toString()).child("username");
        reference.setValue(ed2.getText().toString());
        reference = FirebaseDatabase.getInstance().getReference().child("USERS").child("CUSTOMER").child(ed2.getText().toString()).child("LOC");
        reference.child("latitude").setValue(34.12);
        reference.child("longitude").setValue(78.98);
//        StorageReference filepath = mStorage.child("USERS").child("CUSTOMER").child(ed2.getText().toString()).child("PROFILE");
        DocumentReference docRef = userdatabse.collection(type).document(ed2.getText().toString());
