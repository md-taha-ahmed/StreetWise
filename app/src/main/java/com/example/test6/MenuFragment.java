...
public class MenuFragment extends Fragment implements MenuDialog.MenuDialogListener,  MenuDialogDel.MenuDialogDelListener {
    ...
    public void getProduct(String name) {
        fmenu_product = name;
        flag = 0  ;
        userdb.collection("HAWKERS").document(username).collection("items")
                .get()
                    ...
                    public void onComplete(@NonNull Task<QuerySnapshot> task) {
                        if (task.isSuccessful()) {
                            for (QueryDocumentSnapshot document : task.getResult()) {
                                item_name = document.getId();
                                if(fmenu_product.equals(item_name)){
                                    flag = 1;
                                    userdb.collection("HAWKERS").document(username).collection("items").document(fmenu_product)
                                            .delete()
                                            ...
                                            .addOnFailureListener(new OnFailureListener() {
                                                @Override
                                                public void onFailure(@NonNull Exception e) {
                                                    Toast.makeText(getActivity().getApplicationContext(), fmenu_product+" does not exist!", Toast.LENGTH_SHORT).show();
                                                }
                                            });
                                }
                            }
